...
public class OptionsBarController {
	...
	public void initialize() {
		...
		fastForwardButton.setImage(new Image(OptionsBarController.class.getResourceAsStream(PokemonTypeGameApplication.FAST_FORWARD_IMAGE_PATH)));
		...
		muteButton.imageProperty().bind(Bindings.createObjectBinding(() -> {
			if (PokemonTypeGameApplication.mutedProperty.get())
				return new Image(OptionsBarController.class.getResourceAsStream(PokemonTypeGameApplication.MUTE_IMAGE_PATH));
			else
				return new Image(OptionsBarController.class.getResourceAsStream(PokemonTypeGameApplication.UNMUTE_IMAGE_PATH));
